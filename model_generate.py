...
def surf_normals_surf_intersect(surf1, surf1_normals, surf2, surf2_normals, epsilon=1e-6):
    """Calcualte intersection points between a surface and rays defined by normal vectors of other surface\n
    Params:\n
...
def surf_to_shell_interp(surf2_array,step=[3,3],sigma=2,pixel_size=1,return_surf=False):
    ...
    # xRow = helpers.getFuncUniformSpacing(np.mean(np.mean(surf2_array,axis=3),axis=0), np.max(np.min(surf2_array[:,0,:,:],axis=1)), np.min(np.max(surf2_array[:,0,:,:],axis=1)), step[0], 0, False)
    # zCol = np.array([np.arange(np.max(np.min(surf2_array[:,2,:,:],axis=2)),np.min(np.max(surf2_array[:,2,:,:],axis=2)),step[1])])
    # xArr = np.repeat(np.array([xRow]),zCol.shape[1],axis=0).transpose()
    # zArr = np.repeat(zCol,xRow.shape[0],axis=0)
    ...
    # points1 = np.array((surf2_array[0,0,:,:].ravel(),surf2_array[0,2,:,:].ravel())).transpose()
    # yArr1Flat = scipy.interpolate.griddata(points1,surf2_array[0,1,:,:].ravel(), xi)
    # points2 = np.array((surf2_array[1,0,:,:].ravel(),surf2_array[1,2,:,:].ravel())).transpose()
    # yArr2Flat = scipy.interpolate.griddata(points2,surf2_array[1,1,:,:].ravel(), xi)

    # yArr1 = yArr1Flat.reshape(xArr.shape)
    # yArr2 = yArr2Flat.reshape(xArr.shape)

    # surf2_array = np.array([[xArr,yArr1,zArr],[xArr,yArr2,zArr]])
...
def fem_model_shell(shell_array, part_file_path, save_path, shell_idx=1):
    ...
    # contents[nsetIdx] = f"*Nset, nset=BCdown-{shell_idx}, generate\n"
    ...
    # contents[nsetIdx] = f"*Nset, nset=BCtop-{shell_idx}, generate\n"
...
if __name__ == "__main__":
    ...
    for i in range(int(len(surf_array_list)/2)):
        ...
        surf2_array = surf2_array[:,:,:,50:]
        [surf2_array, shell_array] = surf_to_shell_interp(surf2_array,step=[6,6],sigma=2,pixel_size=pixelSize,return_surf=True)
    ...
    # vwl.save_multSurf2vtk('data/surfaces/slicewise_z300-750_12_resized.vtk', surf_array_list)
    # vwl.save_multSurf2vtk('data/surfaces/slicewise_z300-750_12_center_resized.vtk', shell_array_list)

    partFilePath = "data/abaqusShells/dummyPart.inp"
    masterFilePath = "data/abaqusShells/dummyMaster.inp"
    savePath = "data/abaqusShells/H29_slicewise_z300-750_2x/"
